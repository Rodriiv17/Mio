    ...
    internal class EjemploOperador1
    {
        ...
        internal void Ejecutar()
        {
            ...
            IEnumerable<Empleado> Empleados = new List<Empleado>()
         {
            ...
            //List<Empleado> empleadosDesarrolloTelMad = Empleados
            //                                 .Where(empleado =>
            //                                           (empleado.Departamento == Departamento.Desarrollo ||
            //                                           empleado.Departamento == Departamento.Soporte)
            //                                            && empleado.Apellidos.StartsWith("C")
            ...
            //var Empleadosportelefono = (from empleado in Empleados where (empleado.Apellidos.StartsWith("C")) orderby empleado.Nombre ascending select empleado.Telefono).ToList();

            //var Empleadosoportelefono1 = (from empleado in Empleados where (empleado.Apellidos.StartsWith("C"))
            //                              orderby empleado.Nombre ascending
            //                              select new { Telefono = empleado.Telefono, Ciudad = empleado.Ciudad }).ToList();

            //foreach (var resultado in Empleadosoportelefono1)
            //{
            ...
            var meternuevalista = Empleados.Concat(empleadosNuevos);
            Empleados = meternuevalista.ToList();

            foreach ( var empleadillo in Empleados)
            {
